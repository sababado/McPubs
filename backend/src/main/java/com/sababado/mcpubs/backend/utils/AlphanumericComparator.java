...
public class AlphanumericComparator implements Comparator<Pub> {
    ...
    @Override
    public int compare(final Pub o1, final Pub o2) {
        String s1 = o1.getFullCode();
        String s2 = o2.getFullCode();

        if (!StringUtils.isEmptyOrWhitespace(o1.getVersion())) {
            s1 += o1.getVersion();
        }
        if (!StringUtils.isEmptyOrWhitespace(o2.getVersion())) {
            s2 += o2.getVersion();
        }
